...
- Let each generation enable or disable automatic recall, with recall enabled by
  default and supplemental memory search still available when it is disabled.
...
- Give generation detail a dedicated memory view that organizes recalled,
  searched, and saved memory by semantic kind and distinguishes creates from
  updates.
...
- Automatic recall is a persisted, default-on generation setting. Disabling it
  suppresses the generation-start prelude and its recall telemetry without
  disabling pinned `search_memory`, closeout, or live memory writes.
...
- Generation detail presents memory separately from the raw execution stream:
  automatic callbacks, standing context, likely relevant memory, supplemental
  search results, and successful creates or updates are grouped by semantic
  type. “Presented” memory is not claimed as article use without separate
  evidence.
