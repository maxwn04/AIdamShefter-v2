...
Frozen query derivations establish identity, completion and source observations.
The reporter adapter executes the query once, selects source objects in stable
traversal order and registers immutable copies in a run-local catalog. Invocation
...
the durable generation evidence when recording is enabled.

The brief consumer resolves immutable records through `EvidenceReader`; it does
