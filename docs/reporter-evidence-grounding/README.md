# Reporter evidence and grounding

Executed frozen-data tools populate a run-local evidence catalog. The model sees
bounded selected records and resolvable references; the existing
`ToolExecutionResult(result, metadata)` seam records full raw responses privately.
Research facts bind selected fields to those records. A source receipt establishes
