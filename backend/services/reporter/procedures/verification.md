...
- Check the article itself, including its headline and lead, not merely whether the workflow produced facts, storylines, or an outline. A confirmed source/prose contradiction needs a correction even if the verifier reports no traceability error. Correct both a misleading brief claim and any draft passage that depends on it.
- Read the lead, body and closing together for contradictory ranks, results or chronology. Check counts and superlatives against the full relevant population, including ties and any qualifying members omitted from the selected examples. Verify the actual earlier endpoint before publishing a rank transition, streak or claim of early dominance.
- Check each trade asset from the named team's perspective: sent stays sent and received stays received. Before finalizing, compare `verify_artifact` directional review cards with the linked full paragraphs, resolving pronouns and counterparties explicitly. Cards use selected source records, so correct the draft or saved claim if either reverses those movements. Name matches only retrieve context; they do not prove the prose, and unmatched or truncated passages need direct review. A net gain in draft-pick count cannot alone justify “spent future flexibility”; count also does not prove value. Confirm that a quoted record belongs to the named team and stated period.
...
- Separate additions from drops and trades from waivers. For claimed payoff, check the exact player's identity, acquiring fantasy team, lineup role, scoring period and transaction timing; a same-surname player or a later move cannot explain an earlier win. Bench points alone do not establish a legal superior lineup or an avoidable pregame mistake.
- Read records in their returned units and periods. Bonus standings decisions cannot explain a contradictory head-to-head winner. For playoff coverage, inspect the bracket relevant to each claimed outcome, including consolation or placement games when covered; keep recorded advancement distinct from a score comparison. Missing bracket information proves neither elimination nor lack of a clinch.
