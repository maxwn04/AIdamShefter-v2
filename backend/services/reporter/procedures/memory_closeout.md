...
- Retain original subjects and add stable selectors for newly central teams in the updated question or summary. Linking an event does not add its participants as storyline subjects. Use the default merge behavior; omit unchanged optional fields while providing the required headline and current summary. Subject replacement is for an intentional relationship correction, never a workaround for name-resolution failure.
- Prepare the complete intended update before selecting it. Save each distinct matchup or trade event, inspect successful receipts, then update the arc once with all selected event IDs. A rejected event is unsaved: repair it or deliberately omit it before updating the arc. Keep broad standings comparisons in the arc rather than packing several games into one event. Check that event headline and summary agree with the source-derived identities and every asset direction, including pick ownership; structured validation does not certify authored prose.
